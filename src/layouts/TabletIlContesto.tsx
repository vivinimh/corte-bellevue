...
function Content() {
  const [ref, isInView] = useInView();
  return (
    <div ref={ref} className={`basis-0 content-stretch flex flex-col gap-[24px] grow items-start min-h-px min-w-px relative shrink-0 reveal-in-view ${isInView ? 'is-in-view' : ''}`} data-name="content">
      <Image />
      <ParagraphContent />
    </div>
  );
...
function Title() {
  const [ref, isInView] = useInView();
  return (
    <h2 ref={ref} className={`basis-0 block font-['EB_Garamond:Regular',sans-serif] font-normal grow leading-[1.15] min-h-px min-w-px relative self-stretch shrink-0 text-[#714b55] text-[32px] reveal-in-view ${isInView ? 'is-in-view' : ''}`}>Dove le eccellenze italiane sono di casa</h2>
  );
}
...
function Image1() {
  const [ref, isInView] = useInView();
  return (
    <div ref={ref} className={`aspect-[672/672] basis-0 content-stretch flex flex-col grow items-start min-h-px min-w-px relative shrink-0 reveal-in-view ${isInView ? 'is-in-view' : ''}`} data-name="Image">
